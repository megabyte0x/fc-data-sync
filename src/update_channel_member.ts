...
import { ChannelMembersResponse } from './types/channel_member.types';
...
async function fetch_user_channels_with_retry(fid: string, retryCount = 0): Promise<ChannelMembersResponse | null> {
    try {
        const options = { method: 'GET', headers: { 'x-api-key': process.env.NEYNAR_API_KEY ?? "", "Content-Type": "application/json", } };

        const response = await axios.get(`https://api.neynar.com/v2/farcaster/user/memberships/list/?limit=100&fid=${fid}`, options);
        const data: ChannelMembersResponse = response.data;
        return data;
    } catch (error) {
        if (retryCount < MAX_RETRIES) {
...
async function process_and_update_chunk(fids: string[]) {
    try {
        for (const fid of fids) {
            const res = await fetch_user_channels_with_retry(fid);
            if (res) {
                ...
                const { data, error } = await supabase
                    .from('users')
                    .update({
                        channels_member: res.members.map((member) => {
                            // update_lead(channel.lead);
                            return {
                                url: member.channel.url,
                                name: member.channel.name,
                                description: member.channel.description,
                                image_url: member.channel.image_url,
                            }
                        })
                    })
                    .eq('fid', fid);
                ...
                if (error) {
                    console.error(`Error updating user channels member for fid ${fid}:`, error);
                } else {
                    console.log(`Successfully updated channels member for fid ${fid}`);
                }
            }
...
async function update_user_channels_member() {
...
async function main() {
    await update_user_channels_member();
}
